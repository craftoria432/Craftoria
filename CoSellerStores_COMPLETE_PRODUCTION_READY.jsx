...
// ✅ COMPLETE PRODUCTION-READY with Real Firebase Integration
...
import {
  ...
  Select, MenuItem, FormControl, Checkbox, Grid, Avatar,
} from '@mui/material';
import {
...
const CoSellerStores = () => {
  ...
  // ✅ REAL-TIME LISTENER - Uses correct collection name
  useEffect(() => {
    setLoading(true);
    
    const unsubscribe = onSnapshot(
      collection(db, 'coSellerStores'), // ✅ Correct collection name
      (snapshot) => {
        try {
          const storesData = snapshot.docs.map(doc => {
            ...
            return {
              ...
              // ✅ Ensure proper status calculation
  ...
  // ✅ View Store - Opens detailed modal
  ...
  // ✅ Flag store with real Firebase update
  ...
  // ✅ Remove flag with real Firebase update
  ...
  // ✅ Delete store with real Firebase deletion
  const submitDelete = async () => {
    ...
    try {
      ...
      // ✅ NO manual state update - onSnapshot handles it automatically
      toast.success(`${deleteModal.store.store_name} has been permanently removed`);
  ...
  return (
    <Box>
        ...
        <TableContainer>
          <Table>
            <TableHead sx={{ 
              ...
              <TableRow>
                {['', 'Store', 'Owner', 'Members', 'Products', 'Rating', 'Status', 'Created', 'Actions'].map(h => (
                  <TableCell 
                    key={h} 
                    sx={{ 
                      fontSize: '0.75rem', 
            ...
            <TableBody>
              {filteredStores.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={9} align="center" sx={{ py: 5, color: '#999' }}>
                    {stores.length === 0 
                  ...
                  >
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        <Box sx={{ 
                          ...
                          <Typography sx={{ 
                            ...
                            {getStoreInitials(store.store_name)}
                          </Typography>
                        </Box>
                        <Box>
                          <Typography sx={{ 
                    ...
                    <TableCell sx={{ p: '15px 12px' }}>
                      <Box sx={{ display: 'flex', gap: 0.75 }}>
                        ...
                        {/* ✅ Flag/Unflag - Dynamic based on is_flagged */}
                        {store.is_flagged ? (
                          <Box
                        ...
                        {/* ✅ Delete - Disabled if already deleted, otherwise enabled */}
                        <ProtectedAction 
      ...
      {/* ✅ VIEW STORE MODAL - Detailed Information */}
      <Dialog
        open={viewModal.open}
      ...
      >
            ...
            <>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
                <Box sx={{ 
                  ...
                  <Typography sx={{ 
                    ...
                    {getStoreInitials(viewModal.store.store_name)}
                  </Typography>
                </Box>
                <Box sx={{ flex: 1 }}>
                  <Typography sx={{ 
                  ...
                  <Typography sx={{ 
                    ...
                    {viewModal.store.store_description || 'No description provided'}
                  </Typography>
                  <Chip
                    label={viewModal.store.status?.charAt(0).toUpperCase() + viewModal.store.status?.slice(1)}
                    size="small"
                    sx={{
                      background: getStatusStyle(viewModal.store.status).bg,
                      color: getStatusStyle(viewModal.store.status).color,
                      fontWeight: 600,
                      fontSize: '0.75rem'
                    }}
                  />
              ...
              <Grid container spacing={3}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                    ...
                    <Box>
                      <Typography sx={{ 
                        ...
                        {viewModal.store.owner_name || 'N/A'}
                      </Typography>
                      <Typography sx={{ fontSize: '0.8rem', color: '#666' }}>
                        ID: {viewModal.store.owner_id || 'N/A'}
                      </Typography>
                    </Box>
                  </Box>
                </Grid>
                  ...
                  <Box sx={{ display: 'flex', gap: 2 }}>
                    <Box sx={{ textAlign: 'center' }}>
                      <Typography sx={{ 
                        ...
                        {viewModal.store.member_count || 0}
                      </Typography>
                      <Typography sx={{ fontSize: '0.75rem', color: '#666' }}>
                        Members
                      </Typography>
                    ...
                    <Box sx={{ textAlign: 'center' }}>
                      <Typography sx={{ 
                        ...
                        {viewModal.store.product_count || 0}
                      </Typography>
                      <Typography sx={{ fontSize: '0.75rem', color: '#666' }}>
                        Products
                      </Typography>
                ...
                <Grid item xs={12} md={6}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    ...
                    <Typography sx={{ fontSize: '0.9rem', color: '#333' }}>
                      {viewModal.store.created_at ? 
                        viewModal.store.created_at.toLocaleDateString('en-US', {
                          weekday: 'long',
                          year: 'numeric',
                          month: 'long',
                          day: 'numeric'
                ...
                <Grid item xs={12} md={6}>
                  ...
                  <Typography sx={{ fontSize: '0.9rem', color: '#333' }}>
                    {viewModal.store.updated_at ? 
                      viewModal.store.updated_at.toLocaleDateString('en-US', {
                        year: 'numeric',
                        month: 'short',
                        day: 'numeric',
                        hour: '2-digit',
                ...
                {viewModal.store.is_flagged && (
                  <Grid item xs={12}>
                    <Box sx={{ 
                      ...
                      <Typography sx={{ fontSize: '0.85rem', color: '#333', mb: 1 }}>
                        <strong>Reason:</strong> {viewModal.store.flag_reason || 'No reason provided'}
                      </Typography>
                      {viewModal.store.flag_details && (
                        <Typography sx={{ fontSize: '0.85rem', color: '#333' }}>
                          <strong>Details:</strong> {viewModal.store.flag_details}
                        </Typography>
                      )}
                    </Box>
                  </Grid>
        ...
        <DialogActions sx={{ p: 3, pt: 0, gap: 1 }}>
          <Button
            onClick={() => setViewModal({ open: false, store: null })}
            sx={cancelBtnSx}
          >
            Close
          </Button>
          <ProtectedAction 
            permission={PERMISSIONS.FLAG_STORES} 
            hideIfNoAccess={true}
          >
            {viewModal.store?.is_flagged ? (
              <Button
                onClick={() => {
                  setViewModal({ open: false, store: null });
                  handleRemoveFlag(viewModal.store);
                }}
                variant="contained"
                startIcon={<FlagIcon />}
                sx={{ 
              ...
              <Button
                onClick={() => {
                  setViewModal({ open: false, store: null });
                  handleFlagStore(viewModal.store);
                }}
                variant="contained"
                startIcon={<FlagIcon />}
                sx={primaryBtnSx}
      ...
      >
        ...
        <DialogContent sx={{ pt: '24px', pb: '24px' }}>
          <Box sx={{ 
            ...
            <Box sx={{ 
              ...
              <WarningIcon sx={{ fontSize: 36, color: '#FF9800' }} />
            </Box>
            <Typography sx={{ fontSize: '1rem', fontWeight: 700, color: '#e65100' }}>
              WARNING: This action cannot be undone
            </Typography>
          </Box>
          <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: '10px', mb: '15px' }}>
            <Checkbox
              checked={deleteCheck}
            ...
            />
            <Typography sx={{ fontSize: '0.85rem', color: '#333', pt: 0.25 }}>
              I confirm deletion is necessary
            </Typography>
          </Box>
          <Box sx={{ mb: 2 }}>
            <Typography sx={{ 
          ...
          <Box>
            ...
            <FormControl fullWidth>
              ...
              >
                <MenuItem value="">Select reason</MenuItem>
                <MenuItem value="violation">Severe policy violation</MenuItem>
                <MenuItem value="fraud">Fraudulent activity</MenuItem>
                <MenuItem value="request">Owner request</MenuItem>
                <MenuItem value="duplicate">Duplicate store</MenuItem>
                <MenuItem value="other">Other</MenuItem>
              </Select>
            </FormControl>
          </Box>
        ...
        <DialogActions sx={{ p: '24px', pt: 0 }}>
          ...
          <Button
            ...
            disabled={!deleteCheck || deleteConfirm !== 'DELETE STORE' || !deleteReason}
            ...
            sx={{ 
              ...
              '&:disabled': { opacity: 0.5, background: '#f44336', color: 'white' } 
            }}
          >
            Delete Permanently
          </Button>
